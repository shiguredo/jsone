...
- 整数化に必要な 10 の冪の指数差には、倍精度で取り得る最大値 632 を上限として設ける。倍精度浮動小数を分解したときの 10 の指数は最小 -325、最大 307 に収まるため指数差は 632 を超えず、この上限は分解の前提が崩れた場合の防御としてだけ働く。上限をこれより小さくすると、10 進では厳密に判定できる組み合わせ（指数差 401 から 632）を誤って倍数ではないと報告するため、上限はこの値にする。上限を超える場合は 10 進の厳密な判定を行わず倍数ではないものとして扱う（倍精度では到達しない防御専用の経路）。`10^632` 程度の bignum なら `rem` 1 回の資源消費が入力データの検証に対して無視できる
...
- 指数差が最大になる組み合わせでも 10 進で厳密に判定する（`#{<<"multipleOf">> => 5.0e-324}` と `1.0e308` が指数差 632 の最大値で `{ok, 1.0e308}`。`#{<<"multipleOf">> => 1.0e-300}` と `1.0e308`、`#{<<"multipleOf">> => 1.0e-93}` と `1.0e308` も `{ok, 1.0e308}`。`#{<<"multipleOf">> => 3.0e-93}` と `1.0e308` は `{error, [#{error := not_multiple_of, ...}]}`）
- 次の入力が引き続き `{ok, Value}` を返す
  - `#{<<"multipleOf">> => 1.5}` と `0`、`4.5`
  - `#{<<"multipleOf">> => 0.0001}` と `0.0075`
- `test/jsone_schema_tests.erl` に `multiple_of_float_test/0` を追加する（整数オペランド、負のオペランド、指数表記、指数差の最大値、2^53 を超える整数、`multipleOf` が正の数値でない場合を含める）
...
`multipleOf` の判定を、倍精度浮動小数の最短往復 10 進表記を正本にした整数判定に置き換えた。

- `jsone_schema_validator` の `is_multiple_of/2` から浮動小数の除算と `trunc/1` をなくし、両オペランドを `decimal_parts/1` で `{整数, 10 の指数}` に分解してから bignum の `rem` で判定する。整数のオペランドは整数のまま扱い、`float_to_list(Value, [short])` は浮動小数のオペランドにだけ使う
- `decimal_parts/1` は仮数部の符号と小数部の桁数を `mantissa_parts/1` で分離し、指数表記も同じ経路で分解する。`pow10/1` は 10 進表記の文字列から bignum を組み立てる
- 指数差の上限は `jsone_schema.hrl` の `?MULTIPLE_OF_EXPONENT_LIMIT` に 632（倍精度で取り得る最大の指数差）を置いた。当初は 400 としていたが、指数差 401 から 632 の帯が 10 進では厳密に判定できるのに倍数ではないと報告される偽陰性になるため、レビューの実測（831,172 組で 55,200 件の不一致、上限 632 で 0 件）を受けて 632 に改めた
- `check_multiple_of/3` の直上のコメントを新しい方針に書き直した
- `test/jsone_schema_tests.erl` に `multiple_of_float_test/0` を追加した（整数オペランド、負のオペランド、指数表記、指数差の最大値 632、2^53 を超える整数、`multipleOf` が正の数値でない場合）

`./rebar3 xref` / `./rebar3 dialyzer` / `./rebar3 as test eunit` / `./rebar3 as test proper` が通り、eunit は 746 件、PropEr は 15 件すべて通過した。`test/JSON-Schema-Test-Suite/tests/draft6` の 702 ケースも引き続き通る。
